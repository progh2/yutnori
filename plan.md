...
윷가락은 반쪽으로 쪼갠 나뭇가지 모양이다. 평평한 쪽이 **배**, 둥근 쪽이 **등**이고,
던져서 **배가 위로 나온 개수**로 결과가 정해진다.

| 결과 | 배가 위 | 확률 | 이동 |
|---|---|---|---|
| 모 | 0개 (전부 등) | 1/16 | +5칸, 추가 턴 |
| 도 | 1개 (표시 없는 윷) | 3/16 | +1칸 |
| 빽도 | 1개 (표시된 윷) | 1/16 | −1칸 |
| 개 | 2개 | 6/16 | +2칸 |
| 걸 | 3개 | 4/16 | +3칸 |
| 윷 | 4개 (전부 배) | 1/16 | +4칸, 추가 턴 |

각 윷가락이 배/등 반반이라고 가정한 이항분포다. 도·빽도를 합치면 도 4/16(=1/4),
개 6/16(=3/8), 걸 4/16(=1/4), 윷·모 각 1/16으로 [위키백과 「윷놀이」](https://ko.wikipedia.org/wiki/윷놀이)의
확률과 같다. 실제 윷가락은 등이 둥글어 굴러 넘어가므로 배가 위로 오는 쪽이 조금 더
잦다고 알려져 있으나(≈60%), 이 게임은 반반으로 단순화한다.

> **설계 결정**: 빽도(뒷도)는 전통 규칙대로 **넷 중 하나에 표시를 하고, 배가 하나만
> 나왔을 때 그것이 표시된 윷이면 빽도**로 판정한다(`MARKED_STICK`). 화면의 윷가락
> 하나에도 붉은 띠를 그려 어느 것이 표시된 윷인지 보이게 한다.
...
- 윷가락은 **평평한 배 + 둥근 등**의 반달 모양으로 만들어, 어느 쪽이 위로 놓였는지
  형태로 알 수 있게 했다 (직육면체였을 때는 색으로만 구분됐다). 표시된 윷에는 붉은 띠
- 네 가락이 회전하며 떨어지고, 배가 위로 나온 개수로 결과(도/개/걸/윷/모/빽도)를 판정한다 —
  화면에 보이는 배의 개수와 표시되는 결과가 항상 일치한다
...
- 키보드만으로도 플레이할 수 있고(스페이스/엔터로 던지기, 숫자키로 말·경로 선택),
...
- **말이 끝나기를 기다린다**: 턴 대사를 읽은 뒤에 윷을 던지고, 결과를 읽은 뒤에 말이
  걷는다(각각 최대 2.2초/1.8초까지만 기다려 흐름이 늘어지지 않게 한다). 연출 속도를
  2배로 올리면 대사가 밀리던 문제를 이렇게 해결했다
- 남성 캐릭터에게는 시스템의 **남성 한국어 음성**을 배정한다. 음성 이름으로 성별을
  가르고(InJoon/혜미 등), 남성 음성이 없으면 피치를 크게 낮춰 대신한다
- 환영 인사는 **정확히 한 번**만 읽는다 — 휴대폰 브라우저는 첫 탭 이전의 발화를 삼키므로,
  삼켜졌을 때만 다시 시도하는 상태 기계(idle → queued → done)로 중복을 막는다
...
- **빽도 판정**: 초안에서는 "도 확률 중 일부를 빽도로 떼는" 단순화를 계획했으나, 구현에서는
  전통 규칙대로 **표시된 윷가락 한 개**를 두고 판정한다 — 배가 하나만 나왔을 때 그것이
  표시된 윷이면 빽도. 결과 확률은 같지만 화면의 윷과 판정 근거가 일치한다 (4.2)
