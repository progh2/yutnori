...
**점수 기반 휴리스틱**으로 구현했다 (`planMove`). 사람 플레이어의 말 선택도 같은 함수를
쓰므로 어떤 말을 옮길지 묻는 UI가 없다.
...
즉 **완주 > 잡기 > 업기 > 전진** 순으로 고른다. 지름길(방 경유) 진입은 사람에게만 묻고,
봇은 거의 항상 이득이므로 자동으로 탄다.
...
- 왼쪽 상단에 **현재 턴 / 다음 턴**과 남은 추가 턴 수를 표시
